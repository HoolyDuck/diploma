...
import { mockAppDetail, mockApplications2 } from "@/lib/mocks";
import { useState } from "react";
import { Header } from "@/components/header";
import { toast } from "sonner";

const appMock = mockAppDetail;
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="col-span-1 md:col-span-3">
            <div>
              <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-4">
                  <img
                    src={appMock.iconMedia?.mediaUrl || appMock.icon}
                    alt={appMock.title}
                  ...
                  <div className="flex flex-col">
                    <h1 className="text-2xl font-bold">{appMock.title}</h1>
                    <div className="flex">
                      <div className="flex items-center gap-2 border-r pr-2 mr-2">
                        <DownloadIcon className="!size-4 text-gray-500" />
                        <span className="text-sm text-gray-600">
                          {appMock.downloads} завантажень
                        </span>
                      </div>
                ...
                <Button disabled>
                  <span>Увійдіть, щоб завантажити</span>
                </Button>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 mb-4">
              {appMock.categories?.map((category, index) => (
                <Badge key={index}>{category.name}</Badge>
              ))}
            </div>

            <div>
              <p className="text-sm text-gray-600">{appMock.description}</p>
            </div>
            <div className="max-w-4xl mt-6 overflow-hidden max-h-[500px] relative border rounded-lg shadow-sm">
              <ImageGallery
                ...
                items={appMock.appMedia!.map((image) => ({
            ...
            <div className="flex flex-col gap-4 mt-6">
              {appMock.reviews!.length > 0 ? (
                appMock.reviews!.map((review) => (
                  <div
                    key={review.id}
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              {mockApplications2.map((appMock, index) => (
                <div
                  key={index}
